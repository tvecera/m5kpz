// https://github.com/VolosR/M5stackVoltmeter

#ifndef STICK_VMETER_H
#define STICK_VMETER_H
...
class Voltmeter {
private:
		static bool i2cReadBytes(uint8_t addr, uint8_t reg_addr, uint8_t *buff, uint16_t len);

		static bool i2cWriteBytes(uint8_t addr, uint8_t reg_addr, const uint8_t *buff, uint16_t len);

		static bool i2cReadU16(uint8_t addr, uint8_t reg_addr, uint16_t *value);

		static bool i2cWriteU16(uint8_t addr, uint8_t reg_addr, uint16_t value);

		static float getResolution(voltmeterGain_t gain);

		static uint16_t getCoverTime(voltmeterRate_t rate);

		static uint8_t getPGAEEEPROMAddr(voltmeterGain_t gain);
...
public:
		explicit Voltmeter(const uint8_t ads1115_addr = ADS115_ADDR, const uint8_t eeprom_addr = EEPROM_ADDR);
		...
		bool isInConversion() const;

		void startSingleConversion() const;

		bool EEPORMRead(uint8_t address, uint8_t *buff, uint8_t len) const;

		bool readCalibrationFromEEPROM(voltmeterGain_t gain, int16_t *hope, int16_t *actual) const;
};

#endif

#endif //STICK_VMETER_H
