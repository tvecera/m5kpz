...
bool Voltmeter::i2cWriteBytes(uint8_t addr, uint8_t reg_addr, const uint8_t *buff, uint16_t len) {
...
Voltmeter::Voltmeter(const uint8_t ads1115_addr, const uint8_t eeprom_addr) {
...
bool Voltmeter::isInConversion() const {
	uint16_t value = 0x00;
...
void Voltmeter::startSingleConversion() const {
	uint16_t reg_value = 0;
	bool result = i2cReadU16(_ads1115_addr, ADS1115_RA_CONFIG, &reg_value);
...
bool Voltmeter::EEPORMRead(uint8_t address, uint8_t *buff, uint8_t len) const {
	return i2cReadBytes(_eeprom_addr, address, buff, len);
}

bool Voltmeter::readCalibrationFromEEPROM(voltmeterGain_t gain, int16_t *hope, int16_t *actual) const {
